...
PS_OUTPUT main(PS_INPUT input)
{
    ...
    for (int ligNo = 0; ligNo < POINT_LIGHT_NUM; ligNo++)
    {
        ...
        // // 拡散反射を計算
        //// 1. 光源からサーファイスに入射するベクトルを計算
        //ligDir = normalize(worldPos - pointLights[ligNo].position2);
        //// 2. 光源からサーフェイスまでの距離を計算
        //distance = length(worldPos - pointLights[ligNo].position2);
        //// 3. 影響率を計算する影響率は0.0～1.0の範囲で、
        ////    指定した距離（pointsLights[i].range）を超えたら、影響率は0.0になる
        //affect = 1.0f - min(1.0f, distance / pointLights[ligNo].range2);
        //// 4. 拡散反射光を加算
        //pointLig += CalcLambertDiffuse(
        //    ligDir,
        //    pointLights[ligNo].color2,
        //    normal) * affect;
